...
pub use field_name::{FieldKind, FieldName};
...
// TODO allow values to be borrowed as well
type FieldMap<S=Box<str>> = IndexMap<FieldName<S>, Vec<u8>>;
...
/// ```
// TODO: want to support Clone but that means Header must also be clone
#[derive(Debug, PartialEq, Eq, Clone)]
// TODO default an owned type for S?
pub struct Header<S> where S: FieldStr {
    version: Version,
    fields: FieldMap<S>,
}

impl<S: FieldStr> Header<S> where FieldName<S>: From<FieldKind> {
    ...
    pub fn parse(mut bytes: &[u8]) -> Result<(Header<&str>, usize), HeaderParseError> {
        let (version_consumed, version) = Version::parse(bytes)?;
        bytes = &bytes[version_consumed..];

        let mut fields: FieldMap<&str> = Default::default();
    ...
    // TODO this can take any borrowed form of a field, S need not match
    pub fn get_field_bytes<F: Into<FieldName<S>>>(&self, field: F) -> Option<&[u8]> {
        let field: FieldName<&str> = field.into();
    ...
    pub fn get_field_bytes_raw<F: Into<FieldName<S>>>(&self, field: F) -> Option<&[u8]> {
    ...
    pub fn set_field<N: Into<FieldName<S>>, V: Into<Vec<u8>>>(
        &mut self,
        name: N,
        value: V,
    ) -> Option<Vec<u8>> {
    ...
    pub fn remove_field<N: Borrow<FieldName<S>>>(&mut self, name: N) -> Option<Vec<u8>> {
    ...
    /// the WARC version.
    pub fn iter_field_bytes(&self) -> impl Iterator<Item = (&FieldName<S>, &[u8])> {
    ...
    /// [`set_field`].
    pub fn iter_field_bytes_mut(&mut self) -> impl Iterator<Item = (&FieldName<S>, &mut Vec<u8>)> {
...
impl Into<Header<Box<str>>> for Header<&str> {
    fn into(self) -> Header<Box<str>> {
        todo!()
    }
}
...
pub(crate) struct Field<S> where S: AsRef<str> {
    name: FieldName<S>,
    value: Vec<u8>,
}
...
pub(crate) fn get_record_header<R: BufRead>(mut reader: R) -> Result<Header<Box<str>>, HeaderParseError> {
    enum FirstChanceOutcome {
        Success(usize, Header<Box<str>>),
        KeepLooking(Vec<u8>),
    }
    ...
    let outcome = {
        ...
        match Header::parse(buf) {
            Ok((parsed, n)) => FirstChanceOutcome::Success(n, parsed.into()),
